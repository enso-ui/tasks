    ...
    <div class="checklists-wrapper">
        <div class="is-multiline mt-3">
            <div class="mt-2"
                 ...
                 :key="checklist.id">
                <checklist-item :id="id"
                    @change-status="createForm = false"
                    :checklist="checklist" :create-form="createForm"
                    ref="checklist"/>
            </div>
        </div>
        <div v-show="createForm" class="mt-3">
            <input class="textarea" v-model="checklist.name"
                      rows="2" placeholder="Add new to-do">
            <div class="mt-3">
                <button class="button is-rounded is-small is-bold mr-1 action" @click="createForm = false">
                    <span>
                        {{ i18n('Cancel') }}
                    </span>
                ...
                <button class="button is-rounded is-small is-bold is-success action" @click="create">
                    <span>
                        {{ i18n('Save') }}
                    </span>
                    <span class="icon">
        ...
        <div id="add-item" class="mt-5 is-size-6 is-align-items-center
                is-clickable" @click="createForm = true;">
             <span class="icon is-small">
                 <fa icon="plus"/>
             </span>
            Add an Item
